...
from lsa_attention import Location_Sensitive_Attention as Attention
...
class Decoder(torch.nn.Module):
    ...
    def Train(self, encodings, lengths, features):
        ...
        prcoessed_memories, alignments, cummulated_alignments, contexts = self.Get_Attention_Initial_States(
            memories= encodings
            )
        ...
        for step in range(features.size(1)):
            ...
            contexts, alignments = self.attention(
                ...
                cumulated_alignments= cummulated_alignments,
                masks= encoding_masks
                )
            cummulated_alignments += alignments
    ...
    def Get_Attention_Initial_States(self, memories):
        ...
        '''
        prcoessed_memories = self.attention.Get_Processed_Memory(memories)   # [Batch, Enc_t, Att_dim], This is to prevent from repeating calculation.
        alignments = self.attention.Get_Initial_Alignment(memories)
        cummulated_alignments = self.attention.Get_Initial_Alignment(memories)
        contexts = memories.new_zeros(
            size= (memories.size(0), memories.size(2))
            )

        return prcoessed_memories, alignments, cummulated_alignments, contexts
